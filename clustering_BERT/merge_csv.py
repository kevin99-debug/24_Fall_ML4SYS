...
def main():
    ...
    graph_embeddings = '../IR_GAE/embedding_vectors.csv'
    for_loop_embeddings = '../IR_BERT/embedding_vectors_all.csv'

    # Validate input file paths
    if not os.path.isfile(graph_embeddings):
        logging.error(f"Graph embeddings file not found: {graph_embeddings}")
        return
    if not os.path.isfile(for_loop_embeddings):
        logging.error(f"Other embeddings file not found: {for_loop_embeddings}")
        return

    # Load and prepare CSV files
    try:
        graph_df, other_df = load_and_prepare_csv(graph_embeddings, for_loop_embeddings)
    except Exception as e:
