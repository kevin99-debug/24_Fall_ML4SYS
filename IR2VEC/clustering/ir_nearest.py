...
def find_nearest_embeddings(centroids, embeddings, n_neighbors=5):
    ...
    """
    logging.info(f"Finding the {n_neighbors} nearest embeddings for each centroid.")
    ...
    logging.info(f"Found nearest embeddings for {centroids.shape[0]} centroids.")
...
def save_ir_file(ir_id, ir_file_path, module_content, graphs_dir):
    ...
    try:
        ...
        if process.returncode != 0:
            logging.error(f"llvm-dis failed for ir_id {ir_id}: {ir_errors.decode('utf-8')}")
            return False

        # Save the IR to the specified path
        with open(ir_file_path, "w") as temp_file:
            temp_file.write(ir_output.decode('utf-8'))
        logging.info(f"Saved IR file to '{ir_file_path}'.")
        ...
        # logging.info(f"Generated call graph for '{ir_file_path}'.")
        ...
        #     logging.info(f"Call graph saved to '{new_dot_file_path}'.")
        # else:
        #     logging.error(f"Expected DOT file '{generated_dot_file}' not found.")
        #     return False

        return True

    except Exception as e:
        logging.error(f"Failed to save IR file for ir_id {ir_id}: {e}")
        return False
...
def build_ir_mapping(nearest_indices, merged_df):
    ...
    for centroid_idx, neighbors in enumerate(nearest_indices):
        ...
        try:
            ...
            logging.error(f"Invalid centroid_ir_id '{centroid_ir_id}' for centroid index {centroid_idx}. Skipping.")
            continue
        ...
        for neighbor_idx in neighbors:
            ...
            try:
                ...
                logging.error(f"Invalid neighbor_ir_id '{neighbor_ir_id}' for centroid {centroid_ir_id}. Skipping.")
                continue
    ...
    logging.info(f"Built IR to centroids mapping with {len(ir_to_centroids)} unique IR IDs.")
...
def process_dataset(ir_to_centroids, dataset, ir_directory, graphs_dir):
    ...
    try:
        ...
        logging.info(f"IR files will be saved to '{ir_directory}'.")
        logging.info(f"Call graph DOT files will be saved to '{graphs_dir}'.")
        ...
        for ir_id, module in enumerate(dataset):
            if ir_id in ir_to_centroids:
                ...
                for centroid_ir_id in centroid_ir_ids:
                    ...
                    if not success:
                        logging.error(f"Failed to process IR file for ir_id {ir_id} under centroid_ir_id {centroid_ir_id}.")
            if ir_id > 20000:
                break

        logging.info("Completed processing the streaming dataset.")
...
def main():
    ...
    # Load embeddings
    combined_df = pd.read_csv(combined_embeddings)
    combined_df['embedding_vector'] = combined_df['embedding_vector'].apply(lambda x: np.array(literal_eval(x))).tolist()

    # Load centroids
    centroids_df = load_embeddings(centroids_csv)

    # Extract numerical arrays from 'parsed_embedding'
    embeddings = np.vstack(combined_df['embedding_vector'].values)
    ...
    logging.info(f"Loading the '{dataset_name}' dataset in streaming mode.")
    try:
        dataset = load_dataset(dataset_name, split='train', streaming=True)
        logging.info("Streaming dataset loaded successfully.")
    except Exception as e:
    ...
    logging.info("Nearest embedding extraction and IR saving process completed successfully.")
